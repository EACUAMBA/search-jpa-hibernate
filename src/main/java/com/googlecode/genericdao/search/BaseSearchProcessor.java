 ...
 * <ol>
 * <li><code>generateQL()</code> - is used for getting the actual search results.</li>
 * <li><code>generateRowCountQL()</code> - is used for getting just the number of results.</li>
 * </ol>
...
public abstract class BaseSearchProcessor {
    ...
    /**
     * This is the string used to represent the root entity of the search within the query. The default value is <code>"_it"</code>. It may
     * be necessary to use a different alias if there are entities in the data model with the name or property "_it".
     */
    ...
    /**
     * Generate the QL string for a given search. Fill paramList with the values to be used for the query. All parameters within the query
     * string are specified as named parameters ":pX", where X is the index of the parameter value in paramList.
     */
    ...
    protected String generateJoin(SearchContext ctx, List<String> customFroms) {
        StringBuilder sb = new StringBuilder();
        for (String cf : customFroms) {
            sb.append(" ");
            appendCustomExpression(sb, ctx, cf);
        }
        return sb.toString();
    }
    ...
    /**
     * Internal method for generating from clause. This method should be called after generating other clauses because it relies on the
     * aliases they create. This method takes every path that is called for in the other clauses and makes it available as an alias using
     * left joins. It also adds join fetching for properties specified by <code>fetches</code> if <code>doEagerFetching</code> is
     * <code>true</code>. <b>NOTE:</b> When using eager fetching, <code>applyFetches()</code> must be executed first.
     */
    ...
    /**
     * Internal method for generating the join portion of the from clause. This method should be called after generating other clauses
     * because it relies on the aliases they create. This method takes every path that is called for in the other clauses and makes it
     * available as an alias using left joins. It also adds join fetching for properties specified by <code>fetches</code> if
     * <code>doEagerFetching</code> is <code>true</code> . <b>NOTE:</b> When using eager fetching, <code>applyFetches()</code> must be
     * executed first.
     */
    ...
    /**
     * Used by {@link #negate(Filter)}. There's a complication with null values in the database so that !(x == 1) is not the opposite of (x
     * == 1). Rather !(x == 1 and x != null) is the same as (x == 1). This method applies the null check explicitly to all filters included
     * in the given filter tree.
     */
    ...
    /**
     * append a custom expression to the string builder, replacing any property tokens (i.e "{prop}") with a reference to the property and
    ...
    protected void appendCustomExpression(StringBuilder sb, SearchContext ctx, String expression, List<?> values) {
        ...
        while (matcher.find()) {
            ...
            if (expression.charAt(matcher.start()) == '{') {
                ...
                if (valueIndex == 0) {
                    throw new IllegalArgumentException("This custom filter expression (" + expression
                            + ") contains a value placeholder for zero (?0), but placeholders should be numbered starting at ?1.");
                }
                if (values == null || values.isEmpty()) {
                    throw new IllegalArgumentException("This custom filter expression (" + expression
                            + ") calls for a value placeholder number " + valueIndex + ", but no values were specified.");
                }
                if (valueIndex > values.size()) {
                    throw new IllegalArgumentException("This custom filter expression (" + expression
                            + ") calls for a value placeholder number " + valueIndex + ", but only " + values.size()
    ...
    /**
     * Given a full path to a property (ex. department.manager.salary), return the reference to that property that uses the appropriate
     * alias (ex. a4_manager.salary).
     */
    ...
    /**
     * Split a path into two parts. The first part will need to be aliased. The second part will be a property of that alias. For example:
     ...
     * <li>Remove filters that require sub-filters but have none (and/or/not/some/all/none)
     * </ol>
     */
                ...
                if (filter.getValue() != null) {
                    if (filter.isTakesListOfSubFilters()) {
                        ...
                        if (!(filter.getValue() instanceof Filter)) {
                            throw new IllegalArgumentException("The search has a filter (" + filter
                                    + ") for which the value should be of type Filter but is of type: " + filter.getValue().getClass());
                        }
                    }
                }
...
//    protected Pattern INJECTION_CHECK = Pattern.compile("^[\\w\\.]*$");
    protected Pattern INJECTION_CHECK = Pattern.compile("(^[\\w\\.]*)([\\s]?)([\\w\\.]*$)");
